...
//  DELETE /api/reminders.php?log=1          → clear reminder log
// ================================================================
...
try {
    ...
    if ($method === 'POST' && $action === 'log') {
        ...
        $stmt->execute([
            ...
            ':inv_num' => $body['invoice_num']          ?? '',
            ':client'  => $body['client_name']          ?? '',
            ':type'    => $body['type']                 ?? 'due_reminder',
        ...
        $detail = ($body['client_name'] ?? '') . ' via ' . ($body['channel'] ?? '');
        $aStmt = $db->prepare(
            'INSERT INTO activity_log (type, label, detail, invoice_id, user_id, ip)
             VALUES (:type,:label,:detail,:inv,:uid,:ip)'
        );
        $aStmt->execute([':type'=>'reminder_sent',':label'=>$label,':detail'=>$detail,
            ':inv'=>(!empty($body['invoice_id'])?(int)$body['invoice_id']:null),':uid'=>$uid,':ip'=>$ip]);
