...
// Soft-delete column: marks payments whose invoice was deleted
try {
...
switch ($method) {
  case 'GET':
    ...
    foreach ($payments as &$p) {
      ...
      // Expose deleted flag as _invoiceDeleted for JS compatibility
      $p['_invoiceDeleted']     = $p['invoice_deleted'];
    }
  ...
  case 'POST':
    ...
    if (!empty($d['invoice_id'])) {
      ...
      // Sum all previous ACTIVE payments for this invoice (exclude invoice_deleted)
  ...
  case 'DELETE':
    ...
    // Fetch payment before marking (for logging)
    $pmtStmt = $db->prepare('SELECT invoice_id, invoice_number FROM payments WHERE id = ?');
    $pmtStmt->execute([$id]);
    $pmt = $pmtStmt->fetch();

    // Soft-delete: mark as invoice_deleted instead of physical DELETE
    // This keeps the payment visible in the payments table with "Invoice Deleted" status
    $db->prepare('UPDATE payments SET invoice_deleted = 1 WHERE id = ?')->execute([$id]);

    logActivity((int)$_SESSION['user_id'], 'delete', 'payment', $id,
      'Invoice deleted — payment record retained for: ' . ($pmt['invoice_number'] ?? ''));
...
switch ($method) {
  ...
  case 'DELETE':
    ...
    // Fetch payment details before deleting (for logging + invoice revert)
    $pmtStmt = $db->prepare('SELECT invoice_id, invoice_number, amount, settlement_discount FROM payments WHERE id = ?');
    $pmtStmt->execute([$id]);
    $pmt = $pmtStmt->fetch();
    ...
    if ($pmt && !empty($pmt['invoice_id'])) {
      $invoiceId = (int)$pmt['invoice_id'];

      // Check if invoice still exists
      $invStmt = $db->prepare('SELECT id, grand_total FROM invoices WHERE id = ?');
      $invStmt->execute([$invoiceId]);
      $inv = $invStmt->fetch();
      ...
      if ($inv) {
        ...
        // Sum remaining payments after deletion
        $remStmt = $db->prepare('SELECT COALESCE(SUM(amount),0) as paid, COALESCE(SUM(settlement_discount),0) as disc FROM payments WHERE invoice_id = ?');
        $remStmt->execute([$invoiceId]);
        $rem = $remStmt->fetch();
        $totalCovered = (float)$rem['paid'] + (float)$rem['disc'];

        if ($totalCovered <= 0) {
          // No payments left at all — revert to Pending
          $db->prepare("UPDATE invoices SET status='Pending' WHERE id = ?")->execute([$invoiceId]);
        } elseif ($grandTotal > 0 && ($grandTotal - $totalCovered) > 0.01) {
          // Partially covered — set to Partial
          $db->prepare("UPDATE invoices SET status='Partial' WHERE id = ?")->execute([$invoiceId]);
        }
        // If still fully covered, leave status as Paid
      }
    }

    logActivity((int)$_SESSION['user_id'], 'delete', 'payment', $id,
      'Deleted payment for invoice: ' . ($pmt['invoice_number'] ?? ''));
    jsonResponse(['success' => true]);
