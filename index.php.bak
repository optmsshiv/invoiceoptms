          ...
          <div class="form-section">
            ...
            <div class="form-grid g2">
              ...
              <div class="field"><label>Service Type</label>
                <div style="display:flex;gap:6px">
                  <select id="f-service-select" onchange="fillServiceFromSelect(this.value);livePreview()" style="flex:1">
                    <option value="">-- Select or type below --</option>
                  </select>
                </div>
                <input id="f-service" placeholder="Or type custom service…" oninput="livePreview()" style="margin-top:5px">
              </div>
...
function resetCreateForm() {
  ...
  const svcSelEl = document.getElementById('f-service-select'); if (svcSelEl) svcSelEl.value = '';
...
function loadInvoiceIntoForm(inv) {
  ...
  const _svcSel = document.getElementById('f-service-select');
  if (_svcSel) { _svcSel.value = inv.service || ''; }
...
function updateServiceDropdown() {
  const s = document.getElementById('f-service-select'); if (!s) return;
  // Group by category
  const byCategory = {};
  ...
  let html = '<option value="">-- Select or type below --</option>';
  if (STATE.products.length === 0) {
    html += '<option disabled>No services yet — add in Services & Products</option>';
  } else {
    Object.entries(byCategory).forEach(([cat, prods]) => {
      html += `<optgroup label="${cat}">`;
      prods.forEach(p => {
        const rate = p.rate > 0 ? ` — ₹${parseFloat(p.rate).toLocaleString('en-IN')}` : '';
        html += `<option value="${p.name}">${p.name}${rate}</option>`;
      });
      html += '</optgroup>';
    });
  }
...
function fillServiceFromSelect(val) {
  if (!val) return;
  const inp = document.getElementById('f-service');
  if (inp) inp.value = val;
  // Also auto-fill rate/GST if product has them and no items yet
  const p = STATE.products.find(x => x.name === val);
  if (p && p.rate > 0 && typeof formItems !== 'undefined' && formItems.length === 1 && !formItems[0].rate) {
    formItems[0].desc     = p.name;
    formItems[0].rate     = parseFloat(p.rate) || 0;
    formItems[0].gst      = parseFloat(p.gst)  || 0;
