...
//  invoice-render-shared.js, invoice-paid-shared.js, clients.js
//  (loaded before this file — see pages/invoices/create.php).
//  For pages/invoices/create.php (New/Edit Invoice — a real page
//  now, not an in-SPA view).
//
//  MPA CHANGE: ?id=123 in the URL edits that invoice (fetches it,
//  then calls loadInvoiceIntoForm() — same as the old SPA's row-menu
//  "Edit" action did); no ?id= starts a blank invoice, matching the
//  old SPA's DOMContentLoaded init (setTodayDates, addItem,
//  updateClientDropdown, updateServiceDropdown — the rest of that
//  original init block, like renderDashboard/renderInvoicesTable/
//  renderClients/renderProducts/renderPayments/renderTemplatesGrid,
//  belonged to other views the old SPA loaded eagerly on startup;
//  those now live on their own pages and aren't needed here).
// ================================================================
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (editId) {
    try {
      const r = await api('api/invoices.php?id=' + editId);
      STATE.editingInvoiceId = editId;
      loadInvoiceIntoForm(normalizeInvoice(r.data));
    } catch (e) {
...
async function cancelInvoiceForm() {
  ...
  window.location.href = '/pages/invoices/invoices.php';
}
...
async function saveInvoice() {
  ...
  try {
    if (!isNewSave) {
      ...
      // MPA FIX: the old SPA called showPage('invoices',...) here immediately
      // — a DOM view-switch that didn't kill the JS context, so the
      // setTimeout()+Swal below could still fire 600ms later on top of it.
      // A real page navigation would cancel that pending timer entirely,
      // so navigation now happens after the promotion flow resolves
      // (or immediately, only when there's no promotion to show).
          ...
          Swal.fire({
            ...
            if (r.isConfirmed && _estPhone) {
              ...
              sendWA(_estPhone, msg, 'estimate_created', _estInv, _estClient)
                ...
                .finally(() => { window.location.href = '/pages/invoices/invoices.php'; });
    ...
    STATE.editingInvoiceId = null;
    ['renderInvoicesTable','renderDashRecent','renderDonutChart','updateDashStats'].forEach(fn => { if (typeof window[fn] === 'function') window[fn](); });
    ...
    const ec = STATE.settings; // flat keys (see shared-data.js loadCoreData note)
    ...
    // MPA FIX: the old SPA just stayed on the same in-page view here, so
    // the fire-and-forget WA/email sends above (and their success/failure
    // toasts) could complete in the background whenever they finished. A
    // real page navigation would abort any of those still in flight and
    // kill their toasts. Giving them a few seconds before navigating away
    // isn't a perfect guarantee, but it's a reasonable window for a
    // same-request send to complete.
    setTimeout(() => { window.location.href = '/pages/invoices/invoices.php'; }, 2500);
...
async function sendEmailForClient(email, name, num, amount, due, service, d) {
  ...
  const sc    = STATE.settings;
  const ec    = sc; // flat keys (see shared-data.js loadCoreData note)
