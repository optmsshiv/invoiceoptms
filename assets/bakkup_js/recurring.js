...
//     logWAMessage/sendWA — the WA subsystem, not built yet. Wrapped
//     in try/catch so a missing function logs a console warning
//     instead of aborting the whole invoice-generation run.
...
async function runRecurringCheck() {
  ...
  for (const s of schedules) {
    ...
    try {
      ...
      // Auto-fire WA if enabled — WA subsystem not built yet, so this
      // is wrapped defensively rather than left to throw and abort
      // the rest of the run for other schedules.
