...
$pageScripts = ['/assets/js/shared-data.js', '/assets/js/recurring.js'];
...
?>
      ...
      <!-- Recurring schedule modal (2-step) — recurring-page-specific -->
          ...
          <div id="rec-step-1" style="padding:20px 22px;display:flex;flex-direction:column;gap:16px;max-height:72vh;overflow-y:auto">
            ...
            <div id="rec-copy-row" style="display:none">
              <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--teal-bg);border:1.5px solid var(--teal-l);border-radius:9px">
                ...
                <select id="rec-copy-select" style="font-size:12px;padding:5px 8px;border:1px solid var(--teal-l);border-radius:6px;background:var(--card);color:var(--text);max-width:210px" onchange="recCopyFromInvoice(this.value)"></select>
              </div>
            </div>
            ...
            <div style="border-radius:10px;border:1px solid var(--border);background:var(--bg);overflow:hidden">
              ...
              <div style="padding:12px 14px;display:grid;grid-template-columns:1fr 1fr;gap:8px">
                <div><div style="font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px">First Invoice</div><div style="font-size:13px;font-weight:700;color:var(--text)" id="rec-prev-first">—</div></div>
                <div><div style="font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px">Next After That</div><div style="font-size:13px;font-weight:700;color:var(--text)" id="rec-prev-next">—</div></div>
                <div><div style="font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px">Due Date</div><div style="font-size:13px;font-weight:700;color:var(--text)" id="rec-prev-due">—</div></div>
                <div><div style="font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px">Total Invoices</div><div style="font-size:13px;font-weight:700;color:var(--teal)" id="rec-prev-count">—</div></div>
              </div>
            </div>
          </div>

          <!-- STEP 2: What to Bill -->
          <div id="rec-step-2" style="display:none;padding:20px 22px;flex-direction:column;gap:16px;max-height:72vh;overflow-y:auto">
            <div class="field">
              ...
              <div style="border:1.5px solid var(--border);border-radius:9px;overflow:hidden">
                <div style="display:grid;grid-template-columns:1fr 65px 110px 75px 32px;background:var(--bg);font-size:10.5px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.6px">
                  <span style="padding:8px 10px">Description</span><span style="padding:8px 6px;text-align:center">Qty</span><span style="padding:8px 6px;text-align:right">Rate (₹)</span><span style="padding:8px 6px;text-align:center">GST %</span><span></span>
                </div>
                <div id="rec-items-list"></div>
              </div>
              <button type="button" onclick="recAddItem()" style="margin-top:8px;padding:6px 14px;border:1.5px dashed var(--teal);border-radius:7px;background:transparent;color:var(--teal);font-size:12px;font-weight:600;cursor:pointer"><i class="fas fa-plus"></i> Add Item</button>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
              <div class="field">
                <label>Discount</label>
                <div style="display:flex;gap:6px">
                  <select id="rec-disc-type" style="width:88px;flex-shrink:0" onchange="recCalcTotals()"><option value="pct">%</option><option value="fixed">₹ Fixed</option></select>
                  <input type="number" id="rec-disc" value="0" min="0" step="0.01" style="flex:1" oninput="recCalcTotals()">
                </div>
              </div>
              <div class="field"><label>Notes <span style="font-size:11px;color:var(--muted)">(optional)</span></label><input type="text" id="rec-notes" placeholder="e.g. Monthly retainer" style="width:100%"></div>
            </div>
            <div style="border-radius:10px;border:1.5px solid var(--border);overflow:hidden">
              <div style="padding:10px 16px;background:var(--bg);border-bottom:1px solid var(--border);font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.6px">Invoice Summary</div>
              <div style="padding:12px 16px;display:flex;flex-direction:column;gap:6px">
                <div style="display:flex;justify-content:space-between;font-size:13px"><span style="color:var(--muted)">Subtotal</span><span id="rec-tot-sub">₹0.00</span></div>
                <div style="display:flex;justify-content:space-between;font-size:13px"><span style="color:var(--muted)">Discount</span><span id="rec-tot-disc" style="color:var(--red)">-₹0.00</span></div>
                <div style="display:flex;justify-content:space-between;font-size:13px"><span style="color:var(--muted)">GST</span><span id="rec-tot-gst">₹0.00</span></div>
                <div style="height:1px;background:var(--border);margin:4px 0"></div>
                <div style="display:flex;justify-content:space-between;font-size:15px;font-weight:800"><span>Per Invoice</span><span style="color:var(--teal)" id="rec-tot-grand">₹0.00</span></div>
                <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--muted)"><span id="rec-tot-count-label">× — invoices</span><span style="font-weight:700;color:var(--text)" id="rec-tot-overall">—</span></div>
              </div>
            </div>
          </div>
