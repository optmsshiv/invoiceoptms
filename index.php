        ...
        <div class="create-form">
          ...
          <div class="form-section">
            ...
            <div class="form-grid g2">
              ...
              <div class="field g-full"><label>Bank Account Details</label><textarea id="f-bank" oninput="livePreview()"></textarea></div>
              <div class="field g-full"><label>Terms & Conditions</label><textarea id="f-tnc" oninput="livePreview()" style="min-height:72px" placeholder="Enter terms and conditions..."></textarea></div>
...
function tplWatermark(d) {
  ...
  const wText = (window.TPL_CUSTOM && TPL_CUSTOM.watermarkText) ? TPL_CUSTOM.watermarkText : 'PAID';
...
function buildTpl4(d, sc, itemsHTML, gstColHeader) {
  ...
  <div style="padding:44px 52px 0">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:24px;border-bottom:2px solid #111">
      <div>${tplLogoHTML(d,sc)}${tplCompanyInfoHTML(sc,'#555','#888')}</div>
      <div style="text-align:right">
...
function confirmPaid() {
  ...
  const payload = {
    ...
    amount:         parseFloat(document.getElementById('paid-amt').value)||parseFloat(inv.amount)||0,
...
function renderDashKpis() {
  ...
  if (waEl) {
    ...
    // WA stats mini-cards
  const _pendWA=STATE.invoices.filter(i=>i.status==='Pending'||i.status==='Overdue').length;
  const _overWA=STATE.invoices.filter(i=>i.status==='Overdue').length;
  const _paidTM=STATE.invoices.filter(i=>{const d=new Date();return i.status==='Paid'&&i.issued&&new Date(i.issued).getMonth()===d.getMonth()&&new Date(i.issued).getFullYear()===d.getFullYear();}).length;
  const _waClientsCount=STATE.clients.filter(c=>c.wa||c.whatsapp||c.phone).length;
  waEl.innerHTML = `
  <div style="margin-bottom:10px;display:flex;gap:8px;flex-wrap:wrap">
    ${[
      {ic:'fa-paper-plane',    col:'#25D366',label:'Need Follow-up',  val:_pendWA,           sub:'pending/overdue'},
      {ic:'fa-exclamation-triangle',col:'#e53935',label:'Overdue Alerts',val:_overWA,         sub:'send now'},
      {ic:'fa-check-circle',   col:'#00897B',label:'Paid This Month', val:_paidTM,            sub:'receipts sent'},
      {ic:'fa-address-book',   col:'#1565C0',label:'WA-Ready Clients',val:_waClientsCount,    sub:'have phone #'},
    ].map(c=>`<div onclick="showPage('whatsapp',null)" style="flex:1;min-width:110px;background:${c.col}0f;border:1.5px solid ${c.col}28;border-radius:10px;padding:9px 11px;cursor:pointer">
  ...
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;background:var(--card);border:1.5px solid #25D36628;border-radius:10px;padding:10px 14px;">
    ...
    ${toggles.map(t=>`
      <div onclick="showPage('whatsapp',null)" style="display:flex;align-items:center;gap:5px;padding:4px 10px;border-radius:20px;cursor:pointer;flex-shrink:0;background:${t.val?'#25D36612':'var(--bg)'};border:1px solid ${t.val?'#25D36630':'var(--border)'}">
        <span>${t.icon}</span>
        <span style="font-size:11px;font-weight:600;color:${t.val?'#1a7a3c':'var(--muted)'}">${t.label}</span>
        <span style="width:5px;height:5px;border-radius:50%;background:${t.val?'#25D366':'#ccc'};flex-shrink:0"></span>
      </div>`).join('')}
  ...
  // WA stats row
  const waStatsEl = document.getElementById('dashWAStats');
  if (waStatsEl) {
    const wa          = STATE.settings.wa || {};
    const waClients   = STATE.clients.filter(c => c.wa || c.whatsapp || c.phone).length;
    const overdueInvs = STATE.invoices.filter(i => i.status === 'Overdue');
    const pendingInvs = STATE.invoices.filter(i => i.status === 'Pending');
    const hasAPI      = !!(wa.token && wa.pid);
    const cards = [
      { label:'WA Contacts',    val: waClients,          sub: 'of '+STATE.clients.length+' clients',     col:'#00897B', bg:'#f8fffe', border:'#b2dfdb', icon:'👥' },
      { label:'Overdue Alerts', val: overdueInvs.length, sub: 'need immediate follow-up',                col: overdueInvs.length>0?'#e65100':'#00897B', bg: overdueInvs.length>0?'#fff8e1':'#f8fffe', border: overdueInvs.length>0?'#ffe082':'#b2dfdb', icon:'⚠️' },
      { label:'Pending Dues',   val: pendingInvs.length, sub: 'awaiting payment',                        col:'#1565C0', bg:'#f8fffe', border:'#b2dfdb', icon:'⏳' },
      { label:'API Status',     val: hasAPI?'Live':'Setup needed', sub: hasAPI?'Messages via Meta API':'Add token & pid', col: hasAPI?'#2e7d32':'#bf360c', bg: hasAPI?'#e8f5e9':'#fbe9e7', border: hasAPI?'#a5d6a7':'#ffab91', icon: hasAPI?'✅':'🔑' },
    ];
    waStatsEl.innerHTML = '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">' +
      cards.map(c =>
        '<div onclick="showPage('whatsapp',null)" style="flex:1;min-width:130px;background:'+c.bg+';border:1.5px solid '+c.border+';border-radius:10px;padding:10px 14px;cursor:pointer;transition:.2s" onmouseover="this.style.boxShadow='0 4px 12px rgba(0,0,0,.08)'" onmouseout="this.style.boxShadow=''">' +
          '<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;display:flex;align-items:center;gap:4px">' +
            '<span>'+c.icon+'</span><span>'+c.label+'</span>' +
          '</div>' +
          '<div style="font-size:20px;font-weight:800;color:'+c.col+'">'+c.val+'</div>' +
...
function populateTemplateForm() {
  ...
  setV('tpl-name-style',   C.companyNameWeight || '800');
...
window.applyTplCustomization = function() {
  ...
  // Read from user-visible panel (tpl-r-*) preferentially, fallback to slider
