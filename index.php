...
<!-- wa_log_helper.js removed: logic now inline -->
<style>
...
function populateWAPage() {
  ...
  // balance_reminder: DOM id uses hyphens but STATE key uses underscores — must set explicitly
...
async function _renderPortalTable(search) {
  ...
  // ── Action button helper — hover via onmouseover ──────────────
  const _pClr = {
  ...
  tbody.innerHTML = pagedRows.map(inv => {
    ...
    // Link pill — blue lighter with hover
    const linkCell = url
    ...
    let viewsCell;
    if (views === null) {
      viewsCell = `<span style="font-size:12px;color:var(--muted)">—</span>`;
    } else if (views === 0) {
      viewsCell = `<span style="display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:6px;font-size:11px;font-weight:600;background:#FFF4E5;color:#B45309;border:1px solid #FBBF24;white-space:nowrap"><i class="fas fa-eye-slash" style="font-size:10px"></i> Never viewed</span>`;
    } else {
      viewsCell = `<div>
    ...
    let expiryCell;
    if (isExpired) {
      expiryCell = `<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;border-radius:6px;font-size:11px;font-weight:600;background:#FEF0EF;color:#C0392B;border:1px solid #F7C1C1;white-space:nowrap"><i class="fas fa-ban" style="font-size:9px"></i> Expired</span>`;
  ...
  if (pgDiv) {
    ...
    const bS = (act, dis) =>
      `width:30px;height:30px;border-radius:7px;border:1px solid ${act?'var(--teal)':'var(--border)'};background:${act?'var(--teal)':'var(--bg)'};color:${act?'#fff':'var(--text)'};cursor:${dis?'default':'pointer'};font-size:12px;font-weight:${act?'700':'400'};opacity:${dis?'.4':'1'};transition:all .15s`;
    ...
    for (let i=1;i<=pages;i++) {
      html += `<button onclick="window._portalPage=${i};_renderPortalTable()" style="${bS(i===_portalPage,false)}">${i}</button>`;
    }
...
function _buildReminderQueue() {
  ...
  if (statsEl) {
    ...
    const card = (ico,bg,bdr,clr,label,val) =>
      `<div style="background:var(--bg2,var(--bg));border:1px solid ${bdr};border-radius:8px;padding:10px 12px;display:flex;align-items:center;gap:10px">
    ...
    statsEl.innerHTML =
      card('fa-exclamation-triangle','#FEF0EF','#F7C1C1','#C0392B','Overdue',  overdueCnt)  +
      card('fa-calendar-day',        '#FFF4E5','#FBBF24','#B45309','Due Today', todayCnt)    +
      card('fa-clock',               '#EEF5FF','#B5D4F4','#185FA5','Upcoming',  upcomingCnt);
  }
