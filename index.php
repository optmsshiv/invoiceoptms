        ...
        <div class="create-form">
          ...
          <div class="form-section">
            ...
            <div class="form-grid g2">
              ...
              <div class="field g-full"><label>Terms & Conditions</label><textarea id="f-tnc" oninput="livePreview()" style="min-height:72px">1. All prices are inclusive of applicable taxes. 2. Disputes must be raised within 7 days of invoice date. 3. This is a computer-generated invoice and does not require a physical signature. 4. Subject to Patna jurisdiction.</textarea></textarea></div>
...
function resetCreateForm() {
  ...
  if (tncEl && !tncEl.value) tncEl.value = STATE.settings.defaultTnC || '1. All prices are inclusive of applicable taxes.
2. Payment due within the specified period.
3. Disputes must be raised within 7 days of invoice date.';
...
function tplQrHTML(d) {
  if (!d.popt || !d.popt.qr || !d.qrUrl) return '';
...
function openPreviewModal(id) {
  ...
  const d = {
    ...
    notes: inv.notes || '',
    bank: inv.bank || inv.bank_details || '',
    tnc: inv.tnc || '',
    status: inv.status,
    sym: inv.currency || '₹',
...
function sendEmailForInvoice(inv) {
  ...
  const bank = inv.bank || inv.bank_details || STATE.settings.defaultBank || '';
  // Build a data-like object from invoice (no form dependency)
  const d    = { bank, tnc: inv.tnc || inv.terms || '', notes: inv.notes || '' };
  sendEmailForClient(c.email || '', c.name || 'Client', num, fmt_money(inv.amount), due, svc, d);
}
