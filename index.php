...
optmstech.in | +91 XXXXX XXXXX</textarea>
    ...
    <div id="page-portal" class="page">
      <div style="max-width:760px">
        <div class="settings-block" style="margin-bottom:18px">
          <div class="sb-title"><i class="fas fa-link" style="color:var(--teal)"></i> Client Portal Links</div>
          <p style="font-size:13px;color:var(--muted);margin-bottom:16px">Each invoice gets a unique read-only shareable link. Clients can view invoice details without logging in.</p>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
            <div class="field"><label>Find Invoice</label>
              <select id="portal-inv-select" onchange="renderPortalLink()" style="width:100%">
          ...
          <div id="portal-link-box" style="display:none;background:var(--bg);border-radius:10px;padding:16px;border:1.5px solid var(--teal)">
            <div style="font-size:11px;font-weight:700;color:var(--muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px">Shareable Link</div>
            <div style="display:flex;gap:8px;align-items:center">
              <code id="portal-link-url" style="flex:1;font-size:13px;word-break:break-all;color:var(--teal);font-family:var(--mono)"></code>
              <button class="btn btn-outline" onclick="copyPortalLink()" style="flex-shrink:0"><i class="fas fa-copy"></i> Copy</button>
              <button class="btn btn-whatsapp" onclick="sharePortalWA()" style="flex-shrink:0"><i class="fab fa-whatsapp"></i> Share</button>
            </div>
            <div id="portal-inv-preview" style="margin-top:14px;padding-top:14px;border-top:1px solid var(--border)"></div>
          </div>
        </div>
        <!-- All portal links -->
        <div class="settings-block" style="padding:0;overflow:hidden">
          <div class="card-header" style="padding:14px 18px"><span class="card-title">All Invoice Portal Links</span>
    ...
    <div id="page-tax" class="page">
      ...
      <div class="table-card" style="margin-bottom:18px">
        ...
        <table class="data-table"><thead><tr>
          <th>GST Rate</th><th>Taxable Amount</th><th>CGST</th><th>SGST</th><th>IGST</th><th>Total GST</th><th>Invoice Count</th>
      ...
      <div class="table-card">
        ...
        <table class="data-table"><thead><tr>
          <th>Month</th><th>Invoices</th><th>Gross Revenue</th><th>Taxable Value</th><th>GST Collected</th><th>Status</th>
  ...
  if (sel) {
    sel.innerHTML = '<option value="">— Select an invoice —</option>' +
      STATE.invoices.map(inv => {
        const c = STATE.clients.find(x=>String(x.id)===String(inv.client))||{};
        return `<option value="${inv.id}">${inv.num||inv.invoice_number||''} · ${c.name||''} · ${fmt_money(inv.amount||0)}</option>`;
      }).join('');
  }
  _renderPortalTable();
}
...
function _renderTaxRateTable() {
  ...
  _taxInvoices.forEach(inv => {
    ...
    const k = b.gstRate+'%';
    if (!rateMap[k]) rateMap[k] = {rate:k,taxable:0,cgst:0,sgst:0,igst:0,total:0,count:0};
  ...
  tbody.innerHTML = rows.map(r=>`<tr>
    ...
    <td style="font-family:var(--mono)">${fmt_money(r.cgst)}</td>
    <td style="font-family:var(--mono)">${fmt_money(r.sgst)}</td>
    <td style="font-family:var(--mono)">${fmt_money(r.igst)}</td>
...
function _renderTaxMonthlyTable() {
  ...
  _taxInvoices.forEach(inv => {
    ...
    if (!m) return;
    if (!monthMap[m]) monthMap[m]={month:m,count:0,gross:0,taxable:0,gst:0,paid:0};
    const b = _getTaxBreakdown(inv);
    monthMap[m].count++; monthMap[m].gross+=parseFloat(inv.amount||0);
    monthMap[m].taxable+=b.taxable; monthMap[m].gst+=b.gstTotal;
    if(inv.status==='Paid') monthMap[m].paid+=b.gstTotal;
  });
  const rows = Object.values(monthMap).sort((a,b)=>b.month.localeCompare(a.month));
  if (!rows.length) { tbody.innerHTML=`<tr><td colspan="6" style="padding:24px;text-align:center;color:var(--muted)">No data</td></tr>`; return; }
  tbody.innerHTML = rows.map(r=>{
...
function exportTaxCSV() {
  const rows=[['Month','Invoices','Gross Revenue','Taxable Value','GST Collected']];
  const monthMap={};
  _taxInvoices.forEach(inv=>{
    const m=(inv.issued||inv.date||'').slice(0,7); if(!m) return;
    if(!monthMap[m]) monthMap[m]={count:0,gross:0,taxable:0,gst:0};
    const b=_getTaxBreakdown(inv);
    monthMap[m].count++; monthMap[m].gross+=parseFloat(inv.amount||0); monthMap[m].taxable+=b.taxable; monthMap[m].gst+=b.gstTotal;
  });
  Object.entries(monthMap).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([m,r])=>{
    rows.push([m,r.count,r.gross.toFixed(2),r.taxable.toFixed(2),r.gst.toFixed(2)]);
  });
