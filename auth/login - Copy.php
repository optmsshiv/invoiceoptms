...
<div class="wrap" id="login-card">
  ...
  <div class="right">
    ...
    <form method="POST" autocomplete="on" id="login-form">
      ...
      <div class="field stagger" style="animation-delay:.22s">
      ...
      <div class="meta stagger" style="animation-delay:.28s">
        <label class="rem">
      ...
      <button type="submit" class="btn-login stagger" id="signin-btn" style="animation-delay:.34s">
        <i class="fas fa-sign-in-alt" id="btn-icon"></i>
        <span id="btn-label">Sign in</span>
      </button>
    </form>

    <div class="divline stagger" style="animation-delay:.4s"><hr><span>or continue with</span><hr></div>
    <button type="button" class="sso-btn stagger" style="animation-delay:.45s">
...
document.querySelectorAll('.iw input').forEach(input => {
  ...
  input.addEventListener('focus', () => {
    ...
    // restart the animation even if focused again quickly
...
(function(){
  ...
  // Base positions/sizes for the floating docs (unpacked once per resize)
  const docBase = [
    [.07,.12, 50,64,-18,.052, 0],
    [.88,.10, 42,54, 15,.048, 1],
    [.92,.76, 58,74, -9,.052, 2],
    [.05,.80, 38,50, 24,.044, 3],
    [.50,.05, 34,44,  7,.040, 4],
    [.94,.42, 28,36,-14,.034, 5],
    [.20,.92, 26,34, 32,.030, 6],
    [.74,.94, 34,44, -6,.036, 7],
  ];
  const coinBase = [
  ...
  function render(time){
    ...
    // Floating invoice docs — each drifts slowly on its own slightly
    // different sine cycle so they never move in unison
    docBase.forEach(([bx, by, w, h, deg, alpha, i]) => {
      const speed = 0.15 + (i % 4) * 0.03;      // slightly different pace per doc
      const driftX = Math.sin(sec * speed + i) * 9;
      const driftY = Math.cos(sec * speed * 0.8 + i) * 7;
      const driftDeg = Math.sin(sec * speed * 0.6 + i) * 2.5;
      try { drawDoc(W*bx + driftX, H*by + driftY, w, h, deg + driftDeg, alpha); } catch(e){}
    });
