...
// Controls whether Sales/Customers/Products (product-mode) nav &
// pages show — 'service' | 'product' | 'both'.
$businessType   = $settings['business_type']   ?? 'both';
// Products page nav label/wording changes with business type — same
// mapping as the old SPA's BUSINESS_TYPE_LABELS.
$productsNavLabel = [
    'service' => 'Services',
    'product' => 'Products',
    'both'    => 'Services / Products',
][$businessType] ?? 'Services / Products';
// Payments routes to a different page entirely depending on business
// type — richer merged view (with purchase/sale/voucher data) for
// 'product' exactly, simple invoice-payments list otherwise.
$paymentsHref = $businessType === 'product'
...
const SERVER = {
  ...
  // Action-level permissions — owner/super_admin always true, others
  // follow role_permissions. If the action.* keys are missing from the
  // catalog (migration not yet run), default to true so nobody is
  // unexpectedly locked out after deploying.
  canDelete:       <?= json_encode(in_array($userRole, ['owner','super_admin'], true) ? true : (bool)($perms['action.delete']  ?? true)) ?>,
  canArchive:      <?= json_encode(in_array($userRole, ['owner','super_admin'], true) ? true : (bool)($perms['action.archive'] ?? true)) ?>,
  canEdit:         <?= json_encode(in_array($userRole, ['owner','super_admin'], true) ? true : (bool)($perms['action.edit']    ?? true)) ?>,
  canCreate:       <?= json_encode(in_array($userRole, ['owner','super_admin'], true) ? true : (bool)($perms['action.create']  ?? true)) ?>,
  canApproveEdits: <?= json_encode(in_array($userRole, ['owner','admin','super_admin'], true) ? true : (bool)($perms['action.approve_edits'] ?? false)) ?>
};
</script>
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    $navMain = [
        ...
        'customers'     => ['fas fa-address-book',       'Customers',           '/pages/customers/customers.php', 'menu.sales'],
        'stock'         => ['fas fa-warehouse',          'Stock Ledger',        '/pages/stock/stock.php',         'menu.stock'],
        'payments'      => ['fas fa-credit-card',        'Payments',           $paymentsHref,    'menu.payments'],
    ...
    foreach ($navMain as $key => [$icon, $label, $href, $perm]):
        ...
        // Sales applies to product/trading businesses ('product' or 'both').
        if ($key === 'sales' && !in_array($businessType, ['product', 'both'], true)) continue;
        // Customers is narrower — 'product' only, matching the SPA exactly
        // (STATE.settings.businessType === 'product', not extended to 'both').
        if ($key === 'customers' && $businessType !== 'product') continue;
